...
data = pd.read_csv("C:/Users/saidi/Downloads/austin_final_final.csv") 
...
# will serve as the label  
    X_feature = data.drop(['PrecipitationSumInches'], axis = 1) 

# the output or the label. 
    Y_feature = data['PrecipitationSumInches'] 
...
# day 
    day_index = 798
    days = [i for i in range(Y_feature.size)] 
...
# of approx. 2 inches. 
    ...
    plt.scatter(days, Y_feature, color = 'g') 
    plt.scatter(days[day_index], Y_feature[day_index], color ='r') 
    ...
    plt.show() 
    x_vis = X_feature.filter(['TempAvgF', 'DewPointAvgF', 'HumidityAvgPercent', 
...
mainroot.geometry('1920x1080')
root=tk.Frame(mainroot)


heading=tk.Label(mainroot,text="Weather Analysis using time series linear regression",font=("Arial",30,"bold"))
